...
const { CommandCodes, } = EIPPacket;
const DEFAULT_EIP_PORT = 44818;
function setConnectionState(self, state) {
...
export default class EIPLayer extends Layer {
    ...
    listServices(callback) {
        return CallbackPromise(callback, (resolver) => {
            queueUserRequest(this, EIPPacket.ListServicesRequest(this._senderContext), null, (error, reply) => {
                if (error) {
        ...
        return CallbackPromise(callback, (resolver) => {
            ...
            function internalListIdentityReplyHandler(error, reply) {
                if (error) {
                    ...
                    identities.push(reply.items[0].value);
                    clearTimeout(timeoutHandler);
                    if (targetsSpecified) {
    ...
    disconnect(callback) {
        return CallbackPromise(callback, (resolver) => {
            if (this._connectionState !== 0) {
                this.send(EIPPacket.UnregisterSessionRequest(this._sessionHandle, this._senderContext), null, true);
                ...
                for (;;) {
                    ...
                    if (request) {
                        ...
                        if (info && info.connectionID != null) {
                            buffer = EIPPacket.EncodeSendUnitDataMessage(this._sessionHandle, 0, 0, info.connectionID, message);
                            if (!fullMessage) {
                                fullMessage = buffer;
                            }
                            ...
                            buffer = EIPPacket.EncodeSendRRDataMessage(this._sessionHandle, this._senderContext, message);
                            if (!fullMessage) {
                                fullMessage = buffer;
                            }
                            else if (fullMessage.length + buffer.length <= 500) {
        ...
        switch (command) {
            ...
            case CommandCodes.SendRRData: {
                ...
                if (!Array.isArray(packet.items)) {
                    ...
                    const messageItem = packet.items.find((item) => item.type.code === CPF.ItemTypeIDs.UnconnectedMessage);
            ...
            case CommandCodes.SendUnitData: {
                if (packet.status.code === 0) {
                    const addressItem = packet.items.find((item) => item.type.code === CPF.ItemTypeIDs.ConnectedAddress);
                    const messageItem = packet.items.find((item) => item.type.code === CPF.ItemTypeIDs.ConnectedMessage);
