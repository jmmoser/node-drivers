...
//   TAG: 4
// };
import { CommonServiceCodes, 
// ClassCodes,
GeneralStatusCodes, } from '../../../../core/cip/constants/index';
import { DataType, DataTypeCodes, Encode, EncodeSize, EncodeTo, } from '../../../../core/cip/datatypes/index';
import { DecodeTypedData } from '../../../../core/cip/datatypes/decoding';
import { CallbackPromise } from '../../../../utils';
import { Logix5000DataTypeCodes, Logix5000ClassCodes, SymbolServiceCodes, SymbolInstanceAttributeCodes, SymbolInstanceAttributeDataTypes, TemplateServiceCodes, TemplateClassAttributeCodes, TemplateClassAttributeDataTypes, TemplateInstanceAttributeCodes, TemplateInstanceAttributeDataTypes, GenericServiceStatusDescriptions, Member, ControllerInstanceAttributeCodes, ControllerInstanceAttributeDataTypes, } from './constants';
    ...
    for (let i = 0; i < members.length; i++) {
        ...
        if (member.type.atomic) {
            ...
            if (member.type.dimensions === 0) {
                ...
                structValues[member.name] = DecodeTypedData(data, offsetRef, iDataType);
...
function sendPromise(self, service, path, data) {
...
async function readTagFragmented(self, path, elements) {
...
//   return res;
// }
function parseTemplateMemberName(data, offsetRef) {
...
function parseTemplateNameInfo(data, offsetRef) {
...
async function getSymbolSize(layer, scope, tag) {
    if (/\[\d+\]$/.test(tag) === false) {
        ...
        if (symbolParts.length === 1) {
            const [typeInfo, arrayDimensionLengthsInfo,] = await layer.readSymbolAttributeList(scope, symbolParts[0], [
                SymbolInstanceAttributeCodes.Type,
            ...
            if (typeInfo.value && typeInfo.value.dimensions > 0) {
                if (Array.isArray(arrayDimensionLengthsInfo.value)
...
export default class Logix5000 extends CIPLayer {
    ...
    readModifyWriteTag(tag, ORmasks, ANDmasks, callback) {
        return CallbackPromise(callback, (resolver) => {
            ...
            if ((!Array.isArray(ORmasks) && !Buffer.isBuffer(ORmasks))
            ...
            send(this, service, path, requestData, (error, reply) => {
                if (error) {
                    ...
                    try {
                        const { data } = reply;
                        const offsetRef = { current: 0 };
                        const attributeCount = data.readUInt16LE(offsetRef.current);
                        offsetRef.current += 2;
                        const attributeResults = [];
                        for (let i = 0; i < attributeCount; i++) {
                            const code = data.readUInt16LE(offsetRef.current);
                            offsetRef.current += 2;
                            const status = data.readUInt16LE(offsetRef.current);
                            offsetRef.current += 2;
                            if (status !== 0) {
    ...
    async readSymbolAttributesAll(scope, tag, callback) {
        return CallbackPromise(callback, async (resolver) => {
            ...
            const path = encodeFullSymbolPath(scope, symbolID);
            send(this, service, path, undefined, (error, reply) => {
                if (error) {
                    resolver.reject(error, reply);
                }
                else {
    ...
    readTemplate(templateID, callback) {
        return CallbackPromise(callback, async (resolver) => {
            try {
                ...
                let reqOffset = 0;
                for (;;) {
                    const reqData = Encoding.EncodeReadTemplateServiceData(reqOffset, bytesToRead - reqOffset);
                ...
                for (let i = 0; i < attributes[TemplateInstanceAttributeCodes.MemberCount]; i++) {
                    if (offsetRef.current < dataLength - 8) {
                        ...
                        const info = data.readUInt16LE(offsetRef.current);
                        offsetRef.current += 2;
                        const type = data.readUInt16LE(offsetRef.current);
                        offsetRef.current += 2;
                        const memberOffset = data.readUInt32LE(offsetRef.current);
                        offsetRef.current += 4;
                ...
                send(this, service, path, requestData, (error, reply) => {
                    if (error) {
                        ...
                        try {
                            ...
                            const attrs = Object.fromEntries(attributes);
            ...
            send(this, service, path, requestData, (error, reply) => {
                if (error) {
                    ...
                    try {
                        ...
                        const { data } = reply;
                        const offsetRef = { current: 0 };
                        const numberOfAttributes = data.readUInt16LE(offsetRef.current);
                        offsetRef.current += 2;
                        for (let i = 0; i < numberOfAttributes; i++) {
                            const attribute = data.readUInt16LE(offsetRef.current);
                            offsetRef.current += 2;
                            const status = data.readUInt16LE(offsetRef.current);
                            offsetRef.current += 2;
