...
function send(self, internal, message, contextOrCallback) {
...
export default class PCCCLayer extends Layer {
    ...
    wordRangeRead(address, words, callback) {
        ...
        return CallbackPromise(callback, (resolver) => {
            const message = Encoding.EncodeWordRangeReadRequest(this._transactionCounter(), address, words);
    ...
    typedRead(address, items, callback) {
        ...
        return CallbackPromise(callback, (resolver) => {
            ...
            const message = Encoding.EncodeTypedRead(this._transactionCounter(), address, items);
            send(this, true, message, (error, reply) => {
                if (error) {
                    resolver.reject(error, reply);
                }
    ...
    typedWrite(address, value, callback) {
        return CallbackPromise(callback, (resolver) => {
            ...
            const message = Encoding.EncodeTypedWrite(this._transactionCounter(), address, value);
            send(this, true, message, (error, reply) => {
                if (error) {
                    resolver.reject(error, reply);
                }
    ...
    diagnosticStatus(callback) {
        return CallbackPromise(callback, (resolver) => {
            const message = Encoding.EncodeDiagnosticStatus(this._transactionCounter());
            send(this, true, message, (error, reply) => {
                if (error) {
                    resolver.reject(error, reply);
                }
    ...
    echo(data, callback) {
        return CallbackPromise(callback, (resolver) => {
            const message = Encoding.EncodeEcho(this._transactionCounter(), data);
            send(this, true, message, (error, reply) => {
                if (error) {
                    resolver.reject(error, reply);
                }
    ...
    sendNextMessage() {
        ...
        if (request != null) {
            ...
            if (info != null && info.connectionID != null && info.transportHeader != null) {
                packet = Encoding.EncodeConnectedRequest(transaction, info.connectionID, info.transportHeader, message);
            }
            else {
                packet = Encoding.EncodeUnconnectedRequest(transaction, message);
            }
