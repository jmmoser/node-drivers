...
import PDU from '../core/modbus/pdu';
import CreateCounter from '../counter';
const { ReadDiscreteInputs, ReadCoils, ReadInputRegisters, ReadHoldingRegisters, WriteSingleCoil, WriteMultipleCoils, WriteSingleHoldingRegister,
...
export default class Modbus extends Layer {
        ...
        switch (lowerLayer.name) {
            case LayerNames.TCP: {
                ...
                this._send = (pdu, opts, resolver) => {
                    ...
                    const callback = this.contextCallback(once((err) => {
                        if (err) {
                    ...
                    const message = Frames.TCP.Encode(transaction, opts.protocolID || cOpts.protocolID, opts.unitID || cOpts.unitID, pdu);
    ...
    writeSingleHoldingRegister(address, values, callback) {
        return writeRequest(this, WriteSingleHoldingRegister, address, values, this._littleEndian, callback);
    }
