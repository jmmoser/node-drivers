...
import { ClassCodes, VendorNames, } from '../constants/index';
...
const LanguageDataType = DataType.TRANSFORM(DataType.STRUCT([DataType.USINT, DataType.USINT, DataType.USINT]), (value) => value.map((v) => String.fromCharCode(v)).join(''));
const InstanceAttribute = Object.freeze({
    VendorID: new CIPAttribute(ClassCodes.Identity, 1, 'Vendor ID', DataType.TRANSFORM(DataType.UINT, (value) => ({
        code: value,
        name: VendorNames[value] || 'Unknown',
    }))),
    DeviceType: new CIPAttribute(ClassCodes.Identity, 2, 'Device Type', DataType.TRANSFORM(DataType.UINT, (value) => ({
        code: value,
        name: DeviceTypeNames[value] || 'Unknown',
    }))),
    ProductCode: new CIPAttribute(ClassCodes.Identity, 3, 'Product Code', DataType.UINT),
    Revision: new CIPAttribute(ClassCodes.Identity, 4, 'Revision', DataType.TRANSFORM(DataType.STRUCT([DataType.USINT, DataType.USINT]), (value) => ({
        major: value[0],
        minor: value[1],
    }))),
    Status: new CIPAttribute(ClassCodes.Identity, 5, 'Status', DataType.TRANSFORM(DataType.WORD, (value) => ({
        code: value,
    ...
    State: new CIPAttribute(ClassCodes.Identity, 8, 'State', DataType.TRANSFORM(DataType.USINT, (value) => InstanceStateDescriptions[value] || 'Reserved')),
