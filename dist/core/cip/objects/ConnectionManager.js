...
(function (ServiceCodes) {
    ServiceCodes[ServiceCodes["ForwardClose"] = 78] = "ForwardClose";
    ServiceCodes[ServiceCodes["UnconnectedSend"] = 82] = "UnconnectedSend";
    ServiceCodes[ServiceCodes["ForwardOpen"] = 84] = "ForwardOpen";
    ServiceCodes[ServiceCodes["GetConnectionData"] = 86] = "GetConnectionData";
    ServiceCodes[ServiceCodes["SearchConnectionData"] = 87] = "SearchConnectionData";
    ServiceCodes[ServiceCodes["GetConnectionOwner"] = 90] = "GetConnectionOwner";
    ServiceCodes[ServiceCodes["LargeForwardOpen"] = 91] = "LargeForwardOpen";
...
function errorDataHandler(buffer, offsetRef, res) {
    if (res.status.type === 'routing') {
        res.remainingPathSize = buffer.readUInt8(offsetRef.current);
        offsetRef.current += 1;
...
function connectionDataResponse(buffer, offsetRef) {
    const ConnectionNumber = buffer.readUInt16LE(offsetRef.current);
    offsetRef.current += 2;
    const ConnectionState = buffer.readUInt16LE(offsetRef.current);
    offsetRef.current += 2;
    const OriginatorPort = buffer.readUInt16LE(offsetRef.current);
    offsetRef.current += 2;
    const TargetPort = buffer.readUInt16LE(offsetRef.current);
    offsetRef.current += 2;
    const ConnectionSerialNumber = buffer.readUInt16LE(offsetRef.current);
    offsetRef.current += 2;
    const OriginatorVendorID = buffer.readUInt16LE(offsetRef.current);
    offsetRef.current += 2;
    const OriginatorSerialNumber = buffer.readUInt32LE(offsetRef.current);
    offsetRef.current += 4;
    const OriginatorOtoTCID = buffer.readUInt32LE(offsetRef.current);
    offsetRef.current += 4;
    const TargetOtoTCID = buffer.readUInt32LE(offsetRef.current);
    offsetRef.current += 4;
    const ConnectionTimeoutMultiplierOtoT = buffer.readUInt8(offsetRef.current);
    offsetRef.current += 1;
    offsetRef.current += 3; // Reserved
    const OriginatorRPIOtoT = buffer.readUInt32LE(offsetRef.current);
    offsetRef.current += 4;
    const OriginatorAPIOtoT = buffer.readUInt32LE(offsetRef.current);
    offsetRef.current += 4;
    const OriginatorTtoOCID = buffer.readUInt32LE(offsetRef.current);
    offsetRef.current += 4;
    const TargetTtoOCID = buffer.readUInt32LE(offsetRef.current);
    offsetRef.current += 4;
    const ConnectionTimeoutMultiplierTtoO = buffer.readUInt8(offsetRef.current);
    offsetRef.current += 1;
    offsetRef.current += 3; // Reserved
    const OriginatorRPITtoO = buffer.readUInt32LE(offsetRef.current);
    offsetRef.current += 4;
    const OriginatorAPITtoO = buffer.readUInt32LE(offsetRef.current);
    offsetRef.current += 4;
    return {
        ConnectionNumber,
        ConnectionState,
...
export default class ConnectionManager {
    static UnconnectedSend(request, route, options) {
        ...
        return new CIPRequest(ServiceCodes.UnconnectedSend, ConnectionManagerEPath, buffer, request, {
            serviceNames: ServiceCodes,
            acceptedServiceCodes: [ServiceCodes.UnconnectedSend, request.service],
            statusHandler: (statusCode, extendedBuffer) => {
                switch (statusCode) {
                    case 1:
    ...
    static ForwardOpen(connection, incrementCounters) {
        ...
        offset = encodeConnectionTiming(data, offset, connection.options.tickTime, connection.options.timeoutTicks);
        ...
        return ConnectionManagerCIPRequest(connection.large ? ServiceCodes.LargeForwardOpen : ServiceCodes.ForwardOpen, data, (buffer, offsetRef) => {
            const OtoTNetworkConnectionID = buffer.readUInt32LE(offsetRef.current);
            offsetRef.current += 4;
            const TtoONetworkConnectionID = buffer.readUInt32LE(offsetRef.current);
            offsetRef.current += 4;
            const ConnectionSerialNumber = buffer.readUInt16LE(offsetRef.current);
            offsetRef.current += 2;
            const OriginatorVendorID = buffer.readUInt16LE(offsetRef.current);
            ...
            const OriginatorSerialNumber = buffer.readUInt32LE(offsetRef.current);
            offsetRef.current += 4;
            const OtoTActualPacketRate = buffer.readUInt32LE(offsetRef.current);
            offsetRef.current += 4;
            const TtoOActualPacketRate = buffer.readUInt32LE(offsetRef.current);
            offsetRef.current += 4;
            const appReplySize = 2 * buffer.readUInt8(offsetRef.current);
            offsetRef.current += 1;
    ...
    static ForwardClose(connection) {
        ...
        return ConnectionManagerCIPRequest(ServiceCodes.ForwardClose, data, (buffer, offsetRef) => {
            const SerialNumber = buffer.readUInt16LE(offsetRef.current);
            offsetRef.current += 2;
            const VendorID = buffer.readUInt16LE(offsetRef.current);
            offsetRef.current += 2;
            const OriginatorSerialNumber = buffer.readUInt32LE(offsetRef.current);
            offsetRef.current += 4;
            const appReplySize = 2 * buffer.readUInt8(offsetRef.current);
    ...
    static GetConnectionData(connectionNumber) {
        ...
        return ConnectionManagerCIPRequest(ServiceCodes.GetConnectionData, data, connectionDataResponse);
    }
    /**
     * CIP Vol1 3-5.5.6
     *
    ...
    static SearchConnectionData(connectionSerialNumber, originatorVendorID, originatorSerialNumber) {
        ...
        return ConnectionManagerCIPRequest(ServiceCodes.SearchConnectionData, data, connectionDataResponse);
    }
