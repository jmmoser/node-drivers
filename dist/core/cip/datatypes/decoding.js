...
import { getBits, unsignedIntegerSize, decodeUnsignedInteger, } from '../../../utils';
export function DecodeTypedData(buffer, offsetRef, inputDataType, ctx) {
    ...
    switch (dataType.code) {
        case DataTypeCodes.BOOL:
            ...
            const integerValue = decodeUnsignedInteger(buffer, offsetRef.current, unsignedIntegerSize(boolDataType.position));
            value = getBits(integerValue, boolDataType.position, boolDataType.position + 1) > 0;
            break;
        case DataTypeCodes.SINT:
            value = buffer.readInt8(offsetRef.current);
            offsetRef.current += 1;
            break;
        ...
        case DataTypeCodes.ITIME:
            value = buffer.readInt16LE(offsetRef.current);
            offsetRef.current += 2;
            break;
        case DataTypeCodes.UINT:
        case DataTypeCodes.WORD:
        case DataTypeCodes.ENGUNIT:
            value = buffer.readUInt16LE(offsetRef.current);
            offsetRef.current += 2;
            break;
        case DataTypeCodes.DINT:
        case DataTypeCodes.TIME:
        case DataTypeCodes.FTIME:
            value = buffer.readInt32LE(offsetRef.current);
            offsetRef.current += 4;
            break;
        case DataTypeCodes.REAL:
            value = buffer.readFloatLE(offsetRef.current);
            offsetRef.current += 4;
            break;
        case DataTypeCodes.UDINT:
        case DataTypeCodes.DWORD:
        case DataTypeCodes.DATE:
            value = buffer.readUInt32LE(offsetRef.current);
            offsetRef.current += 4;
            break;
        case DataTypeCodes.STRING: {
            const length = buffer.readUInt16LE(offsetRef.current);
            offsetRef.current += 2;
            value = buffer.toString('ascii', offsetRef.current, offsetRef.current + length);
        ...
        case DataTypeCodes.STRING2: {
            const length = buffer.readUInt16LE(offsetRef.current);
            offsetRef.current += 2;
            value = buffer.toString('utf16le', offsetRef.current, offsetRef.current + 2 * length);
            offsetRef.current += 2 * length;
            break;
        }
        case DataTypeCodes.STRINGN: {
            const width = buffer.readUInt16LE(offsetRef.current);
            offsetRef.current += 2;
            const length = buffer.readUInt16LE(offsetRef.current);
            offsetRef.current += 2;
            const total = width * length;
            value = buffer.toString('utf16le', offsetRef.current, offsetRef.current + total);
            offsetRef.current += total;
            break;
        }
        case DataTypeCodes.LTIME:
        case DataTypeCodes.LINT:
            value = buffer.readBigInt64LE(offsetRef.current);
            offsetRef.current += 8;
            break;
        case DataTypeCodes.LWORD:
        case DataTypeCodes.ULINT:
            value = buffer.readBigUInt64LE(offsetRef.current);
            offsetRef.current += 8;
            break;
        case DataTypeCodes.LREAL:
            value = buffer.readDoubleLE(offsetRef.current);
            offsetRef.current += 8;
            break;
        case DataTypeCodes.STRUCT: {
            const { members, decodeCallback } = dataType;
            /** Name of members is not known so use array to hold decoded member values */
            value = [];
            const subCtx = {};
            if (typeof decodeCallback === 'function') {
                subCtx.dataTypeCallback = (dt) => decodeCallback(value, dt, dataType);
            }
        ...
        case DataTypeCodes.ARRAY: {
            const { lowerBound, upperBound, itemType } = dataType;
            value = [];
        ...
        case DataTypeCodes.ABBREV_ARRAY: {
            const { length, itemType } = dataType;
            value = [];
            if (length === true) {
        ...
        case DataTypeCodes.EPATH:
            const { length, padded } = dataType;
            value = EPath.Decode(buffer, offsetRef, length, padded);
            break;
        case DataTypeCodes.UNKNOWN: {
            const { length } = dataType;
            value = buffer.slice(offsetRef.current, offsetRef.current + length);
            offsetRef.current += length;
            break;
        }
        case DataTypeCodes.TRANSFORM: {
            const { dataType: innerDataType, decodeTransform } = dataType;
            value = decodeTransform(DecodeTypedData(buffer, offsetRef, innerDataType));
            break;
        }
