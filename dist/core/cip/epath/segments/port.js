 ...
 * -----
 */
import { getBits, unsignedIntegerSize, encodeUnsignedInteger, } from '../../../../utils';
...
export default class PortSegment {
    ...
    static Decode(buffer, offsetRef, segmentCode /* , padded */) {
        ...
        if (getBits(segmentCode, 4, 5)) {
            addressLength = buffer.readUInt8(offsetRef.current);
            offsetRef.current += 1;
        }
        else {
            addressLength = 1;
        ...
        if (tempNumber === 15) {
            number = buffer.readUInt16LE(offsetRef.current);
            offsetRef.current += 2;
        }
        else {
            number = tempNumber;
