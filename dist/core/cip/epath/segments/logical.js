 ...
 */
import { getBits, decodeUnsignedInteger, unsignedIntegerSize, } from '../../../../utils';
var TypeCodes;
(function (TypeCodes) {
    TypeCodes[TypeCodes["ClassID"] = 0] = "ClassID";
    TypeCodes[TypeCodes["InstanceID"] = 1] = "InstanceID";
    TypeCodes[TypeCodes["MemberID"] = 2] = "MemberID";
    TypeCodes[TypeCodes["ConnectionPoint"] = 3] = "ConnectionPoint";
    TypeCodes[TypeCodes["AttributeID"] = 4] = "AttributeID";
    TypeCodes[TypeCodes["Special"] = 5] = "Special";
    TypeCodes[TypeCodes["ServiceID"] = 6] = "ServiceID";
    // Reserved = 7
})(TypeCodes || (TypeCodes = {}));
;
var FormatCodes;
(function (FormatCodes) {
    FormatCodes[FormatCodes["Address8Bit"] = 0] = "Address8Bit";
    FormatCodes[FormatCodes["Address16Bit"] = 1] = "Address16Bit";
    FormatCodes[FormatCodes["Address32Bit"] = 2] = "Address32Bit";
    FormatCodes[FormatCodes["Reserved"] = 3] = "Reserved";
...
var SpecialFormatCodes;
(function (SpecialFormatCodes) {
    SpecialFormatCodes[SpecialFormatCodes["ElectronicKey"] = 0] = "ElectronicKey";
})(SpecialFormatCodes || (SpecialFormatCodes = {}));
;
var ElectronicKeyFormatCodes;
(function (ElectronicKeyFormatCodes) {
    ElectronicKeyFormatCodes[ElectronicKeyFormatCodes["Normal"] = 4] = "Normal";
})(ElectronicKeyFormatCodes || (ElectronicKeyFormatCodes = {}));
;
;
                ...
                case SpecialFormatCodes.ElectronicKey:
                    ...
                    switch (val.format) {
                        case ElectronicKeyFormatCodes.Normal: {
                            ...
                            offset = buffer.writeUInt8((((val.compatibility ? 1 : 0) << 7) | (val.revision.major & 0b1111111)), offset);
...
function DecodeElectronicKey(buffer, offsetRef) {
    ...
    const keyFormat = buffer.readUInt8(offsetRef.current);
    offsetRef.current += 1;
    ...
    offsetRef.current += 2;
    const deviceType = buffer.readUInt16LE(offsetRef.current);
    offsetRef.current += 2;
    const productCode = buffer.readUInt16LE(offsetRef.current);
    offsetRef.current += 2;
    const majorRevisionByte = buffer.readUInt8(offsetRef.current);
    offsetRef.current += 1;
    /**
     * Compatibility bit:
    ...
    const minorRevision = buffer.readUInt8(offsetRef.current);
    offsetRef.current += 1;
    return {
        format,
        vendorID,
    ...
    static Decode(buffer, offsetRef, segmentCode, padded) {
        ...
        if (type === TypeCodes.Special) {
            ...
            if (valueSize > 1 && padded) {
                ...
                const padByte = buffer.readUInt8(offsetRef.current);
                offsetRef.current += 1;
                if (padByte !== 0) {
        ...
        switch (type) {
            case TypeCodes.ClassID:
                return LogicalSegment.CreateClassID(value, format);
            case TypeCodes.InstanceID:
                return LogicalSegment.CreateInstanceID(value, format);
            case TypeCodes.AttributeID:
                return LogicalSegment.CreateAttributeID(value, format);
            case TypeCodes.MemberID:
                return LogicalSegment.CreateMemberID(value, format);
            case TypeCodes.ServiceID:
                return LogicalSegment.CreateServiceID(value, format);
            case TypeCodes.Special:
