...
import { ClassCodes, CommonServiceCodes, GeneralStatusCodes, GeneralStatusDescriptions, } from './constants/index';
...
function DecodeResponse(buffer, offsetRef, options, request, handler) {
    ...
    const serviceCode = buffer.readUInt8(offsetRef.current) & 0x7F;
    offsetRef.current += 1;
    ...
    const statusCode = buffer.readUInt8(offsetRef.current);
    offsetRef.current += 1;
    const statusError = (statusCode !== GeneralStatusCodes.Success
        && statusCode !== GeneralStatusCodes.PartialTransfer);
    /** Number of 16 bit words */
    const extendedStatusSize = buffer.readUInt8(offsetRef.current);
    offsetRef.current += 1;
...
export default class CIPRequest {
    ...
    response(buffer, offsetRef) {
        return DecodeResponse(buffer, offsetRef, this.options, this[RequestMessageSymbol], this[ResponseDataHandlerSymbol]);
    }
}
...
function MultiCreateDataHandler(requests) {
    return (buffer, offsetRef) => {
        ...
        for (let i = 0; i < numberOfReplies; i++) {
            ...
            responses.push(requests[i].response(buffer, requestOffsetRef));
...
export class CIPMultiServiceRequest extends CIPRequest {
    constructor(requests, path) {
        super(CommonServiceCodes.MultipleServicePacket, path || MessageRouterPath, undefined, MultiCreateDataHandler(requests));
