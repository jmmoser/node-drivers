import { encodeUnsignedInteger, unsignedIntegerSize, } from '../../utils';
import { DataType, } from './constants';
import { logicalASCIIAddressInfo } from './shared';
var CommandCodes;
(function (CommandCodes) {
    CommandCodes[CommandCodes["Connected"] = 10] = "Connected";
    CommandCodes[CommandCodes["Unconnected"] = 11] = "Unconnected";
...
export function EncodeDataDescriptor(data, offsetRef, id, size) {
    ...
    if (idLength > 0 && sizeLength > 0) {
        offsetRef.current = data.writeUInt8(((0b1000 | idLength) << 4) | (0b1000 | sizeLength), offsetRef.current);
        offsetRef.current = encodeUnsignedInteger(data, offsetRef.current, id, idLength);
        offsetRef.current = encodeUnsignedInteger(data, offsetRef.current, size, sizeLength);
        return;
    }
